...
export const ChargeDialog = forwardRef(({ onComplete }, ref) => {
  ...
  const { data: _bankList } = useFetch("/api/banks", jsonFetcher);
  const bankList = _bankList ?? [];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/bank/${bankCode}`);
        if (!response.ok) {
          setBank(null);
          return;
        }
  ...
  const branch = useMemo(() => {
    ...
    return bank.branches[branchCode];
  }, [bank, branchCode]);
